...
from my_agents.openai_agents.payment_link_generator_agent import run_agent_with_order as generate_payment_link
...
from my_agents.config import client, MODEL_NAME
...
class OrchestratorAgent:
    ...
    async def _classify_intent(self, message: str) -> str:
        ...
        try:
            ...
            if intent not in valid_intents:
                ...
                if any(word in message.lower() for word in ["hello", "hi", "good", "hola", "buenos", "buenas"]):
                    return "greeting"
                elif "?" in message or any(word in message.lower() for word in ["how", "what", "when", "where", "which", "who", "why", "cómo", "qué", "cuándo", "dónde", "cuál", "quién", "por qué"]):
                    return "query"
                elif any(word in message.lower() for word in ["want", "order", "buy", "get", "quiero", "pedir", "comprar", "traer"]):
                    return "order"
                else:
                    return "other"
    ...
    async def _handle_alternative_response(self, message: str, user_id: str) -> str:
        ...
        try:
            # Here you would process the customer's response about the alternatives
            # and update the pending order
            
            # For simplicity, we assume the customer accepted the alternatives
            ...
            response = (
                f"¡Gracias por confirmar los cambios! Hemos actualizado tu pedido.\n\n"
                f"Total a pagar: ${payment_link_result['total_amount']}\n"
                f"Link de pago: {payment_link_result['init_point']}\n\n"
                f"Una vez realizado el pago, te confirmaremos tu pedido."
            )
    ...
    def _build_alternatives_message(self, out_of_stock: List) -> str:
        """Build a message offering alternatives for out-of-stock products"""
        ...
        for product in out_of_stock:
            ...
            if available > 0:
                message += f"- {name}: Solo tenemos {available} unidades disponibles\n"
            else:
                # Here you could include suggestions for similar products
                message += f"- {name}: No disponible por el momento\n"
        
        message += "\n¿Te gustaría modificar tu pedido o continuar con los productos disponibles?"
        return message
